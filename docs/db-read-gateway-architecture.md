...
- **Hermes executor pod**: runs the model and native tools. It has the `rsi-db` client and an execution-scoped DB-read client token. It does not have Postgres DSNs.
- **`rsi-db` client**: a thin CLI around RSI control-plane APIs. It submits validation/query/status requests and records a local handoff marker after query submission.
...
- **No DB DSNs in Hermes**: the executor gets only `rsi-db` and an execution-scoped control-plane token.
...
- **Deterministic Slack UX**: once `rsi-db query` submits a request, the Hermes run stops normal Slack delivery. The approval/result card is the response.
