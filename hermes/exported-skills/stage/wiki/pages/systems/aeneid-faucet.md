...
The Aeneid Faucet system dispenses testnet tokens to users via a web frontend and a backend server currently hosted on GCP. The team is planning to migrate the backend to AWS and is working on granting access and setting up a domain name for the backend.

## Claims

- The Aeneid Faucet is a web application that dispenses testnet tokens to user wallets. `claim:claim_1_1` `confidence:1.00`
...
- The faucet frontend is hosted at https://aeneid.faucet.story.foundation/. `claim:claim_1_2` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_dc87e72b13ffb646b259a7fc91c395d8` `chunk_id=srcchunk_3f7de3056380c3b0a128e380c3c3f79f` `native_locator=slack:C0547N89JUB:1770083249.859729:1770085370.453109` `source_timestamp=2026-02-03T02:22:50Z`
- The backend server is a Google Cloud Compute instance named use1-aeneid-fuacet with IP address 35.207.0.103. `claim:claim_1_3` `confidence:1.00`
...
- The frontend communicates with the backend by sending requests to http://35.207.0.103:23313/. `claim:claim_1_4` `confidence:1.00`
...
- The backend validates requests, stores wallet addresses in a database table, and periodically sends testnet tokens to those wallets in chronological order. `claim:claim_1_5` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_43c0946f3f4587e464c535b2f02e284a` `chunk_id=srcchunk_42b6f69cb1c483b952a70161fab8aedb` `native_locator=slack:C0547N89JUB:1770083249.859729:1770969770.716529` `source_timestamp=2026-02-13T08:03:17Z`
- The backend server currently does not have a domain name; the frontend uses the IP address directly. `claim:claim_1_6` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_43c0946f3f4587e464c535b2f02e284a` `chunk_id=srcchunk_42b6f69cb1c483b952a70161fab8aedb` `native_locator=slack:C0547N89JUB:1770083249.859729:1770969770.716529` `source_timestamp=2026-02-13T08:03:17Z`
- There is a plan to migrate the backend server from GCP to AWS. `claim:claim_1_7` `confidence:1.00`
...
- To facilitate migration, the team intends to have the frontend use a domain name instead of a hardcoded IP, allowing the backend to be moved without frontend changes. `claim:claim_1_8` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_15036f8cdcd7730d9bf3ef252d63cd08` `chunk_id=srcchunk_b8eb04c30ffaafd6bc68a3ae08061d6b` `native_locator=slack:C0547N89JUB:1770083249.859729:1770971534.533319` `source_timestamp=2026-02-13T08:32:14Z`
- Access to the GCP project (story-aeneid) and AWS account (story-testnet) is required for the migration. `claim:claim_1_9` `confidence:1.00`
...
- GCP access has been granted, but AWS access setup is still in progress. `claim:claim_1_10` `confidence:1.00`
...
- The person needing access uses the email Haodi@piplabs.xyz, but that email appears to be suspended. `claim:claim_1_11` `confidence:1.00`
...
- Is the server migration to AWS complete?
- What domain name will be assigned to the backend?
- What is the status of the Poseidon email for Haodi?
- Who will set up the AWS instance for the faucet backend?

## Sources

- `source_document_id`: `srcdoc_39d343ffa196af216bd4ea35c17da039`
- `source_revision_id`: `srcrev_7a75154558963d681535c20db1ffa060`
