...
The Aeneid faucet provides testnet tokens for the Story Protocol Aeneid testnet. It consists of a frontend web app and a backend service that validates requests and dispatches tokens in batches.
...
- The frontend queries the backend at http://35.207.0.103:23313/ (IP-based). `claim:claim_1_2` `confidence:1.00`
...
- The backend validates frontend requests, adds wallet addresses to a database table, and chronologically batch-sends testnet tokens to user wallets. `claim:claim_1_3` `confidence:1.00`
...
- The backend is hosted on GCP instance `use1-aeneid-fuacet` (us-east1-c) with IP 35.207.0.103. `claim:claim_1_4` `confidence:1.00`
...
- The GCP project for the faucet is `story-aeneid`. `claim:claim_1_5` `confidence:1.00`
...
- The faucet system does not use a domain name for the backend; the frontend hardcodes the IP address. `claim:claim_1_6` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_43c0946f3f4587e464c535b2f02e284a` `chunk_id=srcchunk_42b6f69cb1c483b952a70161fab8aedb` `native_locator=slack:C0547N89JUB:1770083249.859729:1770969770.716529` `source_timestamp=2026-02-13T08:03:17Z`
- There is a migration plan to move the faucet backend from GCP to AWS, and to change the frontend to use a domain instead of an IP address. `claim:claim_1_7` `confidence:1.00`
...
- The AWS target account for migration is `story-testnet`. `claim:claim_1_8` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_2729f1cdf2797dbe69c29554151a9905` `chunk_id=srcchunk_2a3b7fa2db993f52fdcf4a3068729d22` `native_locator=slack:C0547N89JUB:1770083249.859729:1771024705.562899` `source_timestamp=2026-02-13T23:18:25Z`
- Access to both old GCP instance and new AWS environment has been requested for Haodi (Haodi@piplabs.xyz) to assist with migration; GCP access was granted, AWS access was pending as of the last update. `claim:claim_1_9` `confidence:1.00`
...
- What is the current status of AWS access for Haodi?

## Related Pages

- `aeneid-testnet`
- `story-testnet-infrastructure`

## Sources

- `source_document_id`: `srcdoc_39d343ffa196af216bd4ea35c17da039`
- `source_revision_id`: `srcrev_ccd92a0ea84405779b925919635452d9`
