...
The faucet server for the Aeneid testnet, currently hosted on GCP with IP 35.207.0.103, serving token distribution. Planned migration to AWS with a domain-based frontend.

## Claims

- The faucet server is a GCP Compute Engine instance named use1-aeneid-fuacet in project story-aeneid, zone us-east1-c. `claim:claim_1_1` `confidence:1.00`
...
- The frontend queries http://35.207.0.103:23313/ for faucet requests. `claim:claim_1_2` `confidence:1.00`
...
- The IP address 35.207.0.103 is assigned to the GCP instance use1-aeneid-fuacet. `claim:claim_1_3` `confidence:1.00`
...
- The backend validates requests and adds wallet addresses to a DB table, then batch sends testnet tokens chronologically. `claim:claim_1_4` `confidence:1.00`
...
- The server is planned to be migrated from GCP to AWS. `claim:claim_1_5` `confidence:1.00`
...
- Migration requires the frontend to point to a domain name instead of IP to allow migration without disruption. `claim:claim_1_6` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_15036f8cdcd7730d9bf3ef252d63cd08` `chunk_id=srcchunk_b8eb04c30ffaafd6bc68a3ae08061d6b` `native_locator=slack:C0547N89JUB:1770083249.859729:1770971534.533319` `source_timestamp=2026-02-13T08:32:14Z`
- GCP access for project story-aeneid and AWS access for story-testnet are required for the migration. `claim:claim_1_7` `confidence:1.00`
...
- GCP access has been granted to @U0643B9PPC1; AWS access is being set up. `claim:claim_1_8` `confidence:1.00`
...
- What is the status of the AWS instance setup?
...
- `source_revision_id`: `srcrev_0fb99279d76203172aecf9e3818c201b`
