...
source_revision_ids:
  ...
  - "srcrev_7187280d70853d5f36795cbb39cb8260"
...
The Aeneid testnet faucet distributes testnet tokens to users. It has a frontend at https://aeneid.faucet.story.foundation/ and a backend server currently on GCP (35.207.0.103). The team is migrating the backend to AWS and updating the frontend to use a domain name.
...
- The faucet backend server is the GCP instance 'use1-aeneid-fuacet' with IP 35.207.0.103, and the frontend queries http://35.207.0.103:23313/. `claim:claim_1_2` `confidence:1.00`
...
- The backend validates the frontend request, adds wallet addresses to a database table, and then batch sends testnet tokens to the user wallet chronologically. `claim:claim_1_3` `confidence:1.00`
...
- The team plans to migrate the faucet server from GCP to AWS and update the frontend to use a domain name instead of a hardcoded IP. `claim:claim_1_4` `confidence:1.00`
...
- Haodi (@U0643B9PPC1) needs access to both the GCP instance (story-aeneid project) and the AWS account (story-testnet) to perform the migration. `claim:claim_1_5` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_15036f8cdcd7730d9bf3ef252d63cd08` `chunk_id=srcchunk_b8eb04c30ffaafd6bc68a3ae08061d6b` `native_locator=slack:C0547N89JUB:1770083249.859729:1770971534.533319` `source_timestamp=2026-02-13T08:32:14Z`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_7187280d70853d5f36795cbb39cb8260` `chunk_id=srcchunk_fbbe26ab8337e820063516fff3d353f1` `native_locator=slack:C0547N89JUB:1770083249.859729:1771006037.901899` `source_timestamp=2026-02-13T18:07:17Z`
...
- GCP access for Haodi has been granted; AWS access is being worked on. `claim:claim_1_6` `confidence:1.00`
...
- Haodi's piplabs email (Haodi@piplabs.xyz) is suspended, and they may need a poseidon email for login. `claim:claim_1_7` `confidence:1.00`
...
- Has the AWS instance been set up yet?
- What is the procedure for batch sending tokens? (database table structure, cron job, etc.)
- What is the timeline for migrating the faucet server to AWS?
- Will the frontend be updated to a domain before migration, and what domain will be used?
...
- `source_revision_id`: `srcrev_e8de05b1a44b7de06d1dddaf2dc8140f`
