---
title: "Story-API Shard Task Error"
type: "system"
...
# Story-API Shard Task Error

## Summary

Story-API encountered a fatal stream error on shard tasks, reported via Sentry issue STORY-API-ES. The issue has been resolved multiple times.

## Claims

- story-api shard task returned a fatal stream error `claim:claim_1_1` `confidence:1.00`
...
- Romain Magne (romain.magne@piplabs.xyz) marked Sentry issue STORY-API-ES as resolved `claim:claim_1_2` `confidence:1.00`
...
- The error occurred again later `claim:claim_1_3` `confidence:1.00`
...
- The error occurred again a second time `claim:claim_1_4` `confidence:1.00`
...
- Blake Huynh (blake.huynh@storyprotocol.xyz) marked STORY-API-ES as resolved `claim:claim_1_5` `confidence:1.00`
...
- What causes the shard task to return a fatal stream error?
- Why does the error recur after being resolved?

## Sources

- `source_document_id`: `srcdoc_791a98068e8f43b806246cc620f424ab`
- `source_revision_id`: `srcrev_cabdfcb35e610fda866f0299389375ae`
