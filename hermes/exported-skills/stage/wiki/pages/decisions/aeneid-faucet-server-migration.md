...
The GCP server use1-aeneid-fuacet (IP 35.207.0.103) is used by the Aeneid faucet frontend. Plans to migrate it to AWS are underway, requiring a domain instead of an IP and access to both GCP and AWS accounts.

## Claims

- The server use1-aeneid-fuacet (IP 35.207.0.103) is used by the Aeneid faucet frontend. `claim:claim_1_1` `confidence:1.00`
...
- The backend validates requests, adds wallet addresses to a database, and batch sends testnet tokens. `claim:claim_1_3` `confidence:1.00`
...
- We plan to migrate the server from GCP to AWS. `claim:claim_1_4` `confidence:1.00`
...
- To ease migration, the frontend should point to a domain instead of an IP address. `claim:claim_1_5` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_15036f8cdcd7730d9bf3ef252d63cd08` `chunk_id=srcchunk_b8eb04c30ffaafd6bc68a3ae08061d6b` `native_locator=slack:C0547N89JUB:1770083249.859729:1770971534.533319` `source_timestamp=2026-02-13T08:32:14Z`
- Access to both the old GCP and new AWS servers is required for migration. `claim:claim_1_6` `confidence:1.00`
...
- GCP project story-aeneid and AWS account story-testnet are involved. `claim:claim_1_7` `confidence:1.00`
...
- GCP access has been granted to Haodi (Haodi@piplabs.xyz). `claim:claim_1_8` `confidence:1.00`
...
- Has the domain been set up?
- Is migration complete?
- What is the status of AWS instance setup?

## Sources

- `source_document_id`: `srcdoc_39d343ffa196af216bd4ea35c17da039`
- `source_revision_id`: `srcrev_ce779366d7b9fcb62f1c6cd2a14890d4`
