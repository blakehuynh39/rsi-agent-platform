---
title: "Devnet AWS SSM Access for Yao"
type: "open_question"
...
# Devnet AWS SSM Access for Yao

## Summary

Yao Wang requires SSM start-session access to launch and connect to EC2 instances in the AWS devnet account for full sync testing. Current permissions are insufficient, leading to AccessDeniedException.

## Claims

- Yao Wang requested more permissions to launch instances for full sync testing. `claim:claim_2_1` `confidence:1.00`
...
- Yao requested a higher role in the story-devnet-aws GitHub repo to access GitHub Actions. `claim:claim_2_2` `confidence:1.00`
...
- Tai granted Yao the maintain role in the story-devnet-aws repo. `claim:claim_2_3` `confidence:1.00`
...
- Lucas added permissions for Yao in the AWS devnet account via a Terraform PR. `claim:claim_2_4` `confidence:1.00`
...
- After merging the PR, Yao attempted to start an SSM session to an EC2 instance but received AccessDeniedException with two different roles: SystemAdministrator and AWS-QA-Devnet-Access. `claim:claim_2_5` `confidence:1.00`
...
- Lucas asked Yao to message him when online to run a quick test to resolve the SSM access issue. `claim:claim_2_6` `confidence:1.00`
...
- Why does Yao still get AccessDeniedException when attempting SSM start-session despite being granted permissions?

## Related Pages

- `page_devnet-migration-to-aws`
