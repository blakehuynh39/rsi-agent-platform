...
story-api project experiences intermittent 500 errors, spike protection events, and database connection problems.
...
- Spike protection was activated for story-api (spans) on 2026-01-28 12:58:51 UTC. `claim:claim_1_5` `confidence:1.00`
...
- Spike protection was deactivated for story-api (spans) on 2026-01-28 13:30:11 UTC. `claim:claim_1_6` `confidence:1.00`
...
- On 2026-01-13 approx 11:53:41 UTC, POST /hub/users/notifications failed with 500: Failed to list notifications. `claim:claim_1_7` `confidence:1.00`
...
- On 2026-01-13 approx 12:05:20 UTC, POST /api/v3/*any failed with 500: Internal Server Error. `claim:claim_1_8` `confidence:1.00`
...
- On 2026-01-13 approx 14:56:01 UTC, POST /api/v4/search failed with 500: Internal Server Error. `claim:claim_1_9` `confidence:1.00`
...
- POST /api/v4/assets/edges failed with 500: Internal Server Error at timestamp around 1771978937 (2026-02-25 likely). `claim:claim_1_10` `confidence:1.00`
...
- POST /api/v4/licenses/tokens failed with 500: Internal Server Error at the same timestamp block. `claim:claim_1_11` `confidence:1.00`
...
- POST /api/v4/transactions failed with 500: Internal Server Error at around the same time. `claim:claim_1_12` `confidence:1.00`
...
- POST /api/v4/disputes failed with 500: Internal Server Error at around the same time. `claim:claim_1_13` `confidence:1.00`
...
- GET /api/v4/disputes/:disputeId failed with 500: Internal Server Error at around 1771979411 (2026-02-25 14:30:11?). `claim:claim_1_14` `confidence:1.00`
...
- Search errors occurred: 'failed to perform search: all retries failed: failed to unmarshal response: unexpected end of JSON input' at around 1771982460. `claim:claim_1_15` `confidence:1.00`
...
- Another search error: 'failed to perform search: all retries failed: received status code 500'. `claim:claim_1_16` `confidence:1.00`
...
- Database connection error: password authentication failed for user 'postgres' (FATAL #28P01). `claim:claim_1_17` `confidence:1.00`
...
- Database constraint violation: null value in column 'blacklisted' of relation 'ip_assets' violates not-null constraint when creating IP asset. `claim:claim_1_18` `confidence:1.00`
...
- Database constraint violation: null value in column 'id' of relation 'users' violates not-null constraint when creating entity. `claim:claim_1_19` `confidence:1.00`
...
- POST /api/v4/collections failed with 500: Internal Server Error (timestamp around 1772502680). `claim:claim_1_20` `confidence:1.00`
...
- Integer parsing error: strconv.ParseInt: parsing '99999999999999999999999': value out of range (timestamp 1772978546). `claim:claim_1_21` `confidence:1.00`
...
- GET /api/v4/disputes/:disputeId failed with 500: Internal server error (timestamp 1772978567). `claim:claim_1_22` `confidence:1.00`
...
- Root cause of frequent 500 errors and database connection issues not yet identified.

## Sources

- `source_document_id`: `srcdoc_5d3459afcedd0f5ec46a635b1efb5eea`
- `source_revision_id`: `srcrev_a93b1a67eb49a05c4d36b8a096476030`
