...
Migration of the Aeneid faucet server from GCP to AWS, involving frontend domain updates and access provisioning.

## Claims

- The faucet frontend queries http://35.207.0.103:23313/. `claim:claim_1_1` `confidence:1.00`
...
- The server use1-aeneid-fuacet has IP 35.207.0.103 and is hosted in GCP project story-aeneid. `claim:claim_1_2` `confidence:1.00`
...
- The faucet backend validates requests, stores wallet addresses in a database table, and batch sends testnet tokens. `claim:claim_1_3` `confidence:1.00`
...
- The intention is to migrate the faucet server from GCP to AWS and change the frontend to use a domain instead of IP. `claim:claim_1_4` `confidence:1.00`
...
- GCP access was granted to Haodi@piplabs.xyz for the story-aeneid project. `claim:claim_1_5` `confidence:1.00`
...
- What database table is used for the faucet?
- What domain will the frontend be pointed to?
...
- `source_revision_id`: `srcrev_be51d35358a4405b83af78f6a07ede2f`
