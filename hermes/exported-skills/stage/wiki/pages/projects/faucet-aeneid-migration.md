---
title: "Aeneid Faucet Migration from GCP to AWS"
type: "project"
...
# Aeneid Faucet Migration from GCP to AWS

## Summary

Migrate the Aeneid testnet faucet backend from a GCP Compute Engine instance to AWS, and update the frontend to use a domain instead of hardcoded IP.

## Claims

- The Aeneid faucet is served at https://aeneid.faucet.story.foundation/. `claim:claim_1_1` `confidence:1.00`
...
- The faucet frontend queries backend at http://35.207.0.103:23313/. `claim:claim_1_2` `confidence:1.00`
...
- The backend server is a GCP instance named use1-aeneid-fuacet in project story-aeneid, zone us-east1-c, with IP 35.207.0.103. `claim:claim_1_3` `confidence:1.00`
...
- The backend validates frontend requests, adds wallet addresses to a database table, and batch sends testnet tokens. `claim:claim_1_4` `confidence:1.00`
...
- The faucet was developed by the team @subteam^S07ASS1JQNP. `claim:claim_1_5` `confidence:1.00`
  - citation: `source_document_id=srcdoc_39d343ffa196af216bd4ea35c17da039` `source_revision_id=srcrev_dc87e72b13ffb646b259a7fc91c395d8` `chunk_id=srcchunk_3f7de3056380c3b0a128e380c3c3f79f` `native_locator=slack:C0547N89JUB:1770083249.859729:1770085370.453109` `source_timestamp=2026-02-03T02:22:50Z`
- We are migrating the faucet server from GCP to AWS and want the frontend to point to a domain instead of an IP address. `claim:claim_1_6` `confidence:1.00`
...
- Access has been granted to Haodi (U0643B9PPC1) for GCP project story-aeneid, and AWS account story-testnet is pending. `claim:claim_1_7` `confidence:1.00`
...
- Haodi's piplabs email (Haodi@piplabs.xyz) is suspended, preventing login; he may need an alternative account. `claim:claim_1_8` `confidence:1.00`
...
- Confirm that original faucet developers are aware of the migration plan.
- Create the new AWS instance and replicate the backend service.
- Determine which domain the frontend will use after migration.
- Resolve Haodi's account suspension to enable access.

## Sources

- `source_document_id`: `srcdoc_39d343ffa196af216bd4ea35c17da039`
- `source_revision_id`: `srcrev_a4dc85ea6ea439342fecbed04c9bd2ca`
