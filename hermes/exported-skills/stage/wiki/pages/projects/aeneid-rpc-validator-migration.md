...
source_revision_ids:
  ...
  - "srcrev_faefaf0cae75e550b0ef2d719e9b2d0e"
conflict_state: "none"
---
...
Planned migration of Aeneid RPC nodes and validators with zero downtime. Encountered node sync issues resolved by manually adding peers. Schedule changed multiple times, finalized for Monday 3PM BJT.

## Claims

- Migration of Aeneid RPC and validators was initially planned for January 30, 2026 at 11PM PT with no downtime. `claim:claim_1_1` `confidence:1.00`
...
- A question was raised about sharing the migration plan with partners or on social media. `claim:claim_1_2` `confidence:1.00`
...
- After resync, the node geth logged 'Forkchoice requested sync' and 'Number of finalized block is missing' errors. `claim:claim_1_3` `confidence:1.00`
...
- Consensus layer logs showed warnings about pushing finalized payload while evm syncing and rejecting process proposals due to execution engine syncing. `claim:claim_1_4` `confidence:1.00`
...
- Investigation included questioning if snap sync was used and noting that snapshot data might have issues with EL block data syncing. `claim:claim_1_5` `confidence:1.00`
...
- Monitoring system was not fully connected to the validator, so full logs could not be viewed on grafana. `claim:claim_1_6` `confidence:1.00`
...
- The sync issue was resolved by manually adding a peer; after that everything worked fine and migration could proceed. `claim:claim_1_7` `confidence:1.00`
  ...
  - citation: `source_document_id=srcdoc_b27338e7e24e5f35c74b2f2a9cfd8e4d` `source_revision_id=srcrev_faefaf0cae75e550b0ef2d719e9b2d0e` `chunk_id=srcchunk_88716b237df6d096a2b410be287bd1f7` `native_locator=slack:C0547N89JUB:1769674617.908059:1769685807.879029` `source_timestamp=2026-01-29T11:23:27Z`
- The migration date was noticed as incorrect because it had passed; it was changed to today (Jan 29). `claim:claim_1_8` `confidence:1.00`
...
- The date conflicted with the opening ceremony, so it was further changed to Monday 3PM BJT. `claim:claim_1_9` `confidence:1.00`
...
- Validator monitoring integration with grafana is incomplete, limiting log visibility.

## Related Pages

- `aeneid-validator-peer-sync-fix`
