...
## 5c. Re-Review After Fixes (Full Current Review + Previous Finding Reconciliation)

When the author addresses feedback and asks for a re-review, do a full blind review of the current PR state and also reconcile the previous findings. A re-review is not only a delta check. The fresh subagent must read the full current PR diff and changed-file context with the same bug-finding bar as an initial review, then separately verify whether prior findings were fixed, intentionally withdrawn, or remain unresolved.

**Core principle:** the current PR must be safe on its own, not merely changed in the expected places. Previous findings are required reconciliation inputs, not the review scope boundary.
...
1. **Pull latest commits** on the PR (re-clone if `/tmp/` was cleaned between sessions):
...
3. **Inspect the full current PR diff** — use `gh pr diff N`, then read full changed-file context for files with meaningful behavior changes. Re-review must still catch new or previously missed bugs in the current PR, not only validate the lines touched by the latest fix commit.
...
5. **Verify specific previous findings** — check each previously-flagged issue against the current branch state using remote file reads when practical. See `references/remote-file-inspection.md` for the full pattern set.
...
Re-review instruction to include verbatim:
"This is a re-review, but it is not delta-only. First perform a full blind review of the current PR diff and changed-file context as if this were the first review. Then reconcile the previous findings one by one. Report both new/current-pass findings and previous-finding status. Do not approve solely because the previous findings look fixed."
```
...
- For re-reviews only: previous findings, fix commits, CI deltas, author responses, and the explicit full-review-plus-reconciliation instruction above

Output:
- Findings ordered by severity with file, line, evidence, impact, and concrete fix
- Previous-finding status table for re-reviews, separate from current-pass findings
