...
- **Auto-post race: DB results can land before your "awaiting approval" message.** After submitting a query via `POST /internal/db-read/query`, the control plane posts a Slack approval card AND awaits approval. Once the user clicks "Approve once," the Lambda executes the query and the result is auto-posted to the thread — often within seconds. Your own follow-up `send_message` ("awaiting approval, please click Approve") may land *after* the result already appeared, making it look stale. **Mitigation**: after `POST /internal/db-read/query`, poll `GET /internal/db-read/requests/{id}` once before sending any user-facing message. If the state is already `succeeded`, skip the approval prompt and go straight to summarizing the result. If still `validating` or `pending_approval`, then send the approval prompt.
